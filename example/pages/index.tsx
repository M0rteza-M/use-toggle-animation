...
import hookConfig from '../../use-toggle-animation/package.json';
import type {PackageJson} from 'type-fest';
export default function Index() {
  const { name, description } = hookConfig as PackageJson;

  const { name: authorName, url: authorUrl } = hookConfig.author;
  const { url: repositoryUrl } = hookConfig.repository;
  ...
  const { message } = useToggleAnimation(hookSettings);
